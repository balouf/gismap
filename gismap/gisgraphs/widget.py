...
class GismapWidget:
    ...
    """
    def __init__(self):
        self.names = widgets.Textarea(placeholder='Fabien Mathieu', description='Name(s):',
                                      layout=widgets.Layout(width='50%', height='100px'))
        self.dbs = widgets.RadioButtons(options=['HAL', 'DBLP', 'Both'], description='DB(s):',
                                       layout=widgets.Layout(width="80px", max_width="20%"))
        self.size = widgets.IntSlider(value=10, min=1, max=150, step=1, description="Size",
                                     layout=widgets.Layout(width="250px"))
        self.compute = widgets.Button(description='Map!',
                                     layout=widgets.Layout(width='120px', max_width='140px'))
        self._col = widgets.VBox([self.size, self.compute],
        ...
        self.widget = widgets.VBox([self.save_link, self.out,
                                    widgets.HBox([self.names, self._col, self.dbs])])
        display(self.widget)
    ...
    def html(self):
        ...
        if "," in name:
            ...
            lab = EgoMap(name, dbs=dbs)
            lab.build(target=self.size.value)
        return lab.html()

    def compute_function(self, b, show=True):
        full = self.html()
        b64 = base64.b64encode(f"<html><body>{full}</body></html>".encode('utf8')).decode('utf8')
        payload = f'data:text/html;base64,{b64}'
        savename = safe_filename(self.names.value)
        link_html = f"<a href='{payload}' download='{savename}'>Download the GisMap!</a>"
